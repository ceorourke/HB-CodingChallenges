...
class Node(object):
    ...
    def insert(self, new_data):
        ...
        if new_data >= self.data: # go right
            if self.right is None: # add the node here
                self.right = Node(new_data)
            else: # call the function on this node & go deeper
                self.right.insert(new_data)
        else: # go left
            if self.left is None: # add the node here
