...
def has_balanced_brackets(phrase):
    ...
    for char in phrase: # fill up the stack with brackets
      if char in brackets:
        stack.append(char)

    if len(stack) % 2 != 0: # fail fast if odd number of brackets
      return False

    end = len(stack) -1

    for i in range(len(stack) / 2): # loop through the open brackets
      if matching[stack[i]] != stack[end]: # compare to the closed brackets
        return False
      else:
        end -= 1
